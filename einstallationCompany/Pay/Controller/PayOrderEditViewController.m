//
//  PayOrderEditViewController.m
//  com.chengdushanghai.einstallation
//
//  Created by 云位 on 2021/8/24.
//
...
@interface PayOrderEditViewController ()
...
@implementation PayOrderEditViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self addBackItem];
    self.navigationItem.title = @"订单修改";
    ...
    UITapGestureRecognizer *labelTapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectCity)];
    // 2. 将点击事件添加到label上
...
-(void)requstOrderSubmit{
    ...
    [PayService requestOrderSubmit:dic andResultBlock:^(id  _Nonnull data, id  _Nonnull error) {
        if (data) {
